...
public final class GameManager {
    ...
    /** Sets the quiz and resets progress counters. */
    ...
    public Question currentQuestion() {
        ...
        if (index < 0 || index >= quiz.getQuestions().size()) {
            throw new IllegalStateException("Question index out of range: " + index);
        }
    ...
    public boolean hasNext() { return (total() > 0) && (index < total() - 1); }
    public void goNext() { if (hasNext()) index++; }

    public void addCorrect() { correct++; }
    public void addPoints(double p) { points += clamp01(p); }
